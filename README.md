...
[2026-09 全仓审计与观看体验优先设计](docs/architecture/2026-09-08/README.md)已归档，覆盖播放恢复、中国大陆三网源质量、本地数据、遥控器交互、常用功能、迁移与验收。该文档是待实施方案；下列版本记录保留历史语境，当前实现与未解决问题以审计中的代码证据为准。

[2026-09-11 播放演进增量检视](docs/reviews/2026-09-11-playback-increment-review.md)记录设计提交后的实现检视、待修复问题、验证证据和回归要求。

## v3.4.1 高清优先与电视交互修复

[下载 v3.4.1](https://github.com/peterwangze/yourtv/releases/tag/v3.4.1) · [发布说明](docs/releases/v3.4.1.md)

- 优先实测 1920×1080 及以上线路，旧低清稳定记录不再锁定首选源；内置离线分辨率快照，结合本机首帧速度和连续播放表现选线。高清候选失败后允许兼容线路，并明确提示实际低清分辨率。
- 成都使用场景保留三网候选，切换本地网络路径后重新学习线路健康。源实测、实现边界与验收见 [2026-09-17 高清首选源修复](docs/reviews/2026-09-17-fullhd-source-quality.md)。
- 播放界面：左键上一线路、右键下一线路，首尾循环；上下键切换频道，保留设置中的换台反转选项。OK / MENU 打开频道菜单，节目单从菜单顶部进入。
- 修复台号使用内部 ID 导致的超长数字、数字选台受收藏分组影响、时钟与面板重叠、设置初始焦点和按钮宽度异常。
- 搜索支持输入法和遥控器选择结果；节目单保留空状态、日期与手动刷新入口，播放恢复不会抢走面板焦点。
- 验证与截图见 [2026-09-16 界面和遥控器修复](docs/reviews/2026-09-16-tv-ui-and-remote-fixes.md)。以下版本章节为已发布历史。

## 核心特性

- **开箱即用**：内置经过当前线路探测治理的央视/卫视/地方/港澳台聚合快照，安装后先出频道列表，后台再刷新
- **多源聚合**：预置 11 个面向国内网络的互补公共源，后台全部解析完成后一次性聚合（按"央视→卫视→省份→国家"分类合并），不会由并发下载最快的单源覆盖列表；源管理界面可随时添加、删除自定义源
- **三级分级列表**：一级分类 央视/卫视/地方/海外/其他；央视/卫视 直接两级到频道，地方（省份）/海外（国家）/其他（分类）为 一级→地区/分类→频道 三级，遥控器右键/确认下钻、返回键逐级退出，路径提示实时显示当前位置
- **多线路**：排除冷却中的失败线路后优先实测全高清，同档参考本机稳定播放与首帧速度；保留移动/联通/电信候选，失败退避并自动换线（设置中可开关）
- **源格式全兼容**：
  - M3U（`#EXTM3U`/`#EXTINF`）
...
> 以下为 v3.0.0 历史实现；v3.4.0 已移除播放期间的备用播放器预加载和连接预热，以当前观看稳定性为最高优先级。
...
发布版本为 3.4.0（versionCode 340），[下载 APK](https://github.com/peterwangze/yourtv/releases/download/v3.4.0/juyuan_tv_v3.4.0.apk)。本版还包含播放会话协调、恢复预算、坏线重试、前后台/熄屏恢复及首帧事件归因修复，见[修复记录](docs/reviews/2026-09-11-playback-review-fixes.md)。

**已知问题**：冷启动恢复历史稳定线路后，该线路失败时可能黑屏且缺少重试入口；手动导入局域网地址可能被公共源过滤。部分设备场景已验证，但整体验收及大陆三网实测尚未通过，具体覆盖与证据见[模拟器测试报告](docs/reviews/2026-09-11-android-tv-emulator-qa.md)。

针对“列表很多但实际基本无法使用”的反馈重新审计启动、聚合、选线、探测和失败恢复链路，方案与验收标准见 [docs/国内网络源治理与播放优化计划_v340.md](docs/国内网络源治理与播放优化计划_v340.md)：

- **线路健康可恢复**：记录每条线路自己的成功/失败时间、连续失败、延迟 EWMA 和退避截止；真实播放成功立即清除历史失败，空闲单线探测失败与真实播放失败使用不同退避强度，旧版健康数据自动迁移
- **三网选线**：本机实播/探测结果优先于 URL 猜测；未知网络优先官方/公网 CDN，并在最多 8 条候选中保留中国移动、中国联通、中国电信线路各自的兜底；同一主机最多两条
- **修复稳定源索引漂移**：稳定记录只保存当前 URL，刷新聚合后按 URL 重新定位；不再把旧列表中的 `videoIndex` 套到新列表后随机选中另一条线路
- **聚合去随机化**：首装先应用内置快照，11 个默认源全部解析后一次性合并；激活的默认单源不会在启动时覆盖聚合列表；用户自定义单源仍保留直接导入行为
- **观看优先**：播放期间不再全量/批量探测，不创建备用播放器，不预加载下一频道，不预热连接；源列表和节目单自动刷新也会暂停。仅当当前线路已稳定播放、前向缓冲至少 8 秒、最近无缓冲和遥控器交互时，才以 Range 小请求探测一条备用线路；任意按键或缓冲会立即取消请求
- **缓冲抗抖动**：直播缓冲调整为 8-15 秒，首播 1 秒、卡顿恢复 2.5 秒，在国内公网直播常见的短时抖动下优先连续观看
- **源治理与限流**：移除重复拆分表、过期体育赛事聚合、低存活监测源和默认 IPv6 专用表；每频道最多 8 条候选、同一主机最多 2 条，并保留三网兜底
- **缓存与快照升级**：频道缓存升级为 v340，已有安装不会继续优先读取旧坏列表；内置快照按当前全量探测重建，过滤私网/组播、过期回放和全部不可达频道

## v3.3.0 聚合反向关联源架构 + 开箱即用 + 播放/分类/源治理（三轮数据驱动调研）

基于三轮真实数据调研（详见 [docs/体验演进Gap分析与优化方案_v330.md](docs/体验演进Gap分析与优化方案_v330.md) 与 tools/research/）：全量线路探测（3763 条 ×2 轮）、候选源调研（43 仓库 + 67 组镜像探测）、分类误判全量扫描（真实分类器过 3763 条数据），并吸收用户反馈（"进频道几秒后卡住"、"启动无央视分类"、"应聚合分类后反向关联源、频道级换源、自动选优"、"内置预载源开箱即用"）：

- **架构：聚合为唯一列表，源反向关联到频道**：可见列表永远是多源聚合结果，"源管理/菜单切换"不再用单源整体替换列表（改为调整聚合首选源并重新聚合）；`TV.uriSources` 记录每条线路来源，换线面板展示"线路 N [源名] 分辨率 Ping"
- **内置预载源快照（开箱即用）**：新增 `assets/bundled_channels.json`（发布时由 `tools/build_bundled.py` 从实测源生成，1070 频道/1795 线路，按存活率+源质量排序），安装后零网络依赖立即出完整分类（央视/卫视/地方/海外/其他）；后台 24h 节流刷新聚合，失效才更新
- **播放"不卡住"修复**：
  - 缓冲参数 500/2000/300/500 → 1500/6000/750/1500（秒开与抗抖动平衡）
  - 停播换线增加"本次已出画"门槛：首次起播缓冲只交给 8s 首帧看门狗，杜绝"2-3 秒一换"
  - 缓冲事件换线阈值 10s/5 次 → 15s/8 次 + 单频道会话换线上限 5 次
  - **修复 v3.2.0 遗留关键 Bug**：无缝切台角色交换后新主播放器丢失缓冲/停播监控（主/备监听器同时挂载双实例 + self 守卫），实测交换后状态跟踪与断流恢复正常
  - 备用预加载延迟到主播放稳定 3s 后，避免抢带宽
- **自动选优**：线路健康跨会话持久化（24h TTL，死线/延迟记忆）；聚合排序 = 实测健康 > 稳定源 > 源质量分层 > 清晰度 > 延迟；启动兜底频道不再死循环（列表就绪后重指向列表内同频道）
- **分类全量修复**（3763 条真实数据扫描驱动，265 条修正）：
  - 短英文关键词词边界匹配（inter/ary/n tv/ert/rts/ntv 等 40 个），修复 Zhejiang/Dragon TV International→卫视、发现之旅→纪录、Channel U→香港、TDM→澳门、CGNTV→韩国
  - 咪咕"体育-今/明/昨"轮换分组 240+ 条赛事归位"其他-体育"（不再因球队名误分地方/海外）
  - 卫视分组提示纠偏：五星体育→上海、山东教育→山东、苏州/苏州4K→江苏、重温经典→央视
  - 县市映射补充 7 处（灌阳→广西、高台/和政→甘肃、房山→北京、白城/珲春→吉林、江津→重庆）
  - 无分组专题频道归类（发现之旅→纪录、中学生→教育、财富天下→财经、中国天气→天气 等 8 个）
- **源与镜像治理**（实测证据）：
  - 新增 CCSH/IPTV live_lite（卫视/地方覆盖最大扩充）、YueChan GNTV、YanG-1989 Migu（国内 OTT）
  - 移除 live.fanmingming.cn 两源（不可达+路径已删）、vicjl、hououinkami China_v4（0% 存活）；降级 jk2024988 咪咕2/hujingguang
  - GitHub 加速镜像 11→5（实测可用）+ 直连兜底；fanmingming 路径迁移 ipv4.m3u→index.m3u
- 测试：单测 37 项（debug+release）全绿；Android TV 模拟器实测：开箱出分类、CCTV1-5 秒开、无缝切台 0 延迟、断流 8s 内自动换线恢复、搜索/最近观看/收藏/EPG/比例（4:3 留边）全回归

## v3.2.0 播放质量修复 + 功能补齐（二次竞品深度调研驱动）

基于三份证据驱动竞品调研（[电视家](docs/竞品调研_电视家.md) / [星火TV·DIYP 生态](docs/竞品调研_星火TV_DIYP.md) / [其他主流直播APP](docs/竞品调研_其他直播APP.md)），参考兄弟项目 `D:\AI\agent\zcode\android\tv` 的成熟播放/源加载方案，完成：

- **修复播放覆盖与比例（用户实测反馈）**：备用播放器画面不再覆盖主画面——备用视图 z 序移至主视图之下、备用播放器永不自动起播（角色交换后立即停播，消除 READY 后自动播放竞态）、主/备视图统一 FIT+居中布局（根布局改 FrameLayout，消除 LayoutParams 替换丢约束的比例问题）；4:3/21:9 源等比留边
- **播放不等待（对齐成熟方案）**：HLS/DASH/TS 源加载快速失败（1 次尝试、无退避），坏线 1-2s 到达换线逻辑；首帧看门狗 8s（持续缓冲无错误即换线）；播放健康轮询 15s→2s；换线冷却 30s→8s；自动换线默认开启
- **首启/换源秒出**：预置源并行发现（原串行最坏 19×12s → 并行最快源出列表），applyGate 保证只一个源真正应用（列表不闪换、加载页按时隐藏）
- **频道搜索 + 最近观看**：菜单头部"搜索"按钮 / 遥控器搜索键直达；字母数字键实时过滤；输入为空展示「最近观看」（最近 24 条持久化、去重顶置）；搜索结果复用列表视觉（正在播放标识/收藏）
- **定时关机**：设置页 30/60/90/120 分钟，到点提示并退出
- **画面比例选项**：跟随内容 / 16:9 / 4:3 / 铺满，主/备视图同步生效
- **EPG 分日 + 回看标识**：节目单今天/明天切换；已播节目"回看"角标，OK 提示"回看需源支持"（诚实提示，不假装支持）
- **清理与健壮性**：移除上游"测试码"残留入口（含 NPE 隐患对话框）；节流观察者生命周期化；触摸手势一次性引导卡；稳定源保存 30s 节流

测试：60/60 单测全绿（debug+release）；模拟器实测见 [docs/体验演进Gap分析与优化方案_v320.md](docs/体验演进Gap分析与优化方案_v320.md)（含"CCTV1 观看时备用频道 READY 无覆盖"截图证据、4:3 等比验证、无缝切台、搜索/最近观看/设置页实测）。
