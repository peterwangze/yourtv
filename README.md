...
## v2.9.0 频道质量全量治理（漂移/跳台/广告/分类）

- **修复频道漂移与自动跳台（用户反馈核心）**：聚合/刷新后当前频道改用"分类+规范名"重新锚定（原按 id 匹配，聚合后 id 重排导致侧边栏跳回 CCTV1 而实际播放不变）；频道 id 改为稳定哈希（收藏、稳定源、当前频道跨聚合保持一致）；播放失败不再把过期频道对象追加进分组；配置变更不再重建 Activity（消除销毁期 NPE 崩溃）
- **广告/错误源过滤**：过滤广告网络投放域（stream.ads.ottera.tv，实测"韩国电影1/2"为广告流）、蜻蜓FM 广播电台（黑屏无画面）、咪咕点播虚拟直播（wd-virtuallive）、快手视频文件（yximgs）、"图文直播"比分牌、昨日过期赛事（体育-昨天*）；IPTV 与 WebView 同名频道不再互相混入线路（webview:// 不再污染 IPTV 频道）
- **未分类/乱分类治理**：未分类从 115 降到 3（仅剩真正无法归属的付费/通用名）；繁体台名自动转简体归类（萍鄉→江西、東森/華視/民視→台湾、香港有線→香港）；iptv-org 英文地方台归类（Jiangxi/Harbin/QTV-1/BRTV Kaku/Tonghua TV 等 40+ 关键词）；英文卫视写法（Shenzhen Satellite TV/Shaanxi Satelital TV）并入对应卫视；补 100+ 县级台映射（余杭/句容/平湖/江夏/桦甸…）；"XX新闻综合"与"XX新闻综合频道"跨源合并为一条
- **省级主流频道归位**：吉林都市/影视频道、安徽经济生活、青海经视、浙江钱江都市/教科影视/民生休闲、广东影视/新闻、黑龙江都市/影视、金鹰卡通→湖南、优漫卡通→江苏 等全部进入对应省份
- **实测（模拟器）**：19 源聚合 1072 频道（过滤后），噪音残留 0；CCTV1=27、CCTV5=30、CCTV5+=14、CCTV16=17、湖南卫视=20、深圳卫视=21、卡酷少儿=3、金鹰卡通=5；选择 CCTV1 后持续观察无频道跳转，坏线自动跳过；30 项单元测试全绿
- 版本：2.9.0（versionCode 290）

## v3.0.0 多播放器快速加载（启动/切台性能）

- **播放器池（主+备用）**：启动即创建两个 ExoPlayer 常驻复用；备用播放器在所有设备启用（原仅触屏），预加载"下一频道"到 READY 停在首帧；切台到已就绪目标 → 角色交换 0 延迟接管，旧播放器复用为新的备用槽（不再 release/重建）；菜单跳转等冷切复用主播放器实例（模拟器实测建源 66ms）
- **备用播放器质量**：选线与主播放器一致（跳过坏线、质量分最高）；携带每频道 HTTP 头（Referer/User-Agent）；预加载失败标记坏线并换线重试（≤3 次），不打扰主播放；同时预热目标连接（OkHttp 连接池）
- **启动首帧快速恢复**：修复"稳定源失效黑屏 30s"——会话内从未出画时，播放错误忽略 30s 冷却与自动换源开关立即换线；全量线路探测延迟 10s，不抢首帧带宽
- **设置**：新增"快速切台（预加载下一频道）"开关，默认开启，弱机可关
- 详细设计与主流应用调研见 [docs/播放器性能优化设计.md](docs/播放器性能优化设计.md)；版本 3.0.0（versionCode 300）

## v3.1.0 主流电视 App 体验演进（UI/UX 与易用性）

对标电视家/星火TV/DIYP/TiviMate 与 Google TV/Apple TV 等主流电视体验（详见 [docs/主流电视UIUX调研.md](docs/主流电视UIUX调研.md)），在不改播放内核与数据管道的前提下完成 23 项审计发现（F1-F23）中 18 项的修复与体验演进：

- **按键映射回归主流心智**：MENU 单按直达频道菜单、SETTINGS 单按直达设置（原来单按无反应、需 2/4 连击的隐藏手势）；OK 在 EPG 打开时=关闭节目单（不再误弹菜单）；右键=切换线路、列表右键=前进到收藏按钮；触摸单击=信息卡、双击=菜单、长按=EPG
- **正在播放标识**：频道列表当前播放行左侧青色竖条 + 标题加粗，分组行显示青色圆点，切台后随时可找回当前频道
- **收藏反馈**：收藏改为可聚焦心形按钮（右键聚焦、OK 切换），收藏/取消即时变红 + Toast"已收藏/已取消收藏 XX"，不再右键静默误收藏
- **EPG 可读性**：当前节目行高亮 + "现在"徽标 + 居中定位，空节目单显示空态文案；adapter 懒初始化不再每次重建
- **错误/加载页**：错误页修复双重 inflate（4K 缩放适配恢复），新增"重试"按钮与"按 OK 重试"提示；加载页增加品牌 Logo 与"正在加载频道列表…"文案（置于播放器之上不再被遮挡）
- **信息卡**：只显示进行中节目，带"HH:mm-HH:mm 节目名"时间范围；位图复用消除快速切台内存抖动
- **设置页**：按"播放与解码/显示与导航/收藏与启动/关于与退出"语义分组，字号 14→16sp、行高 48dp；"退出"由开关改为按钮 + 确认对话框；修复 compactMenu 每次启动被强制重置
- **焦点视觉统一**：统一青色描边+半透明填充焦点样式（列表/分组/EPG/设置/菜单设置按钮）；删除未引用的粉色/蓝色焦点资源
- **菜单头部"设置"入口**：SETTINGS 键被系统拦截的设备（如 Google TV）也能进入设置
- **修复启动聚合 ANR（重要性能回归）**：频道分类/合并/排序原在主线程执行（上千频道×数十次关键词匹配），聚合窗口内按任意键即 ANR；现在分类结果缓存化 + 纯计算移入后台线程，实测聚合窗口内连续按键无 ANR
- **数字直拨空安全**：数据未就绪时按数字键提示"加载中"而非崩溃；线路列表编号 1 起始 + 域名摘要 + 完整 URL 分行显示
- 版本 3.1.0（versionCode 310）；设计与测试报告见 [docs/体验演进Gap分析与优化方案.md](docs/体验演进Gap分析与优化方案.md)
...
> 注：v2.4.5 起移除了 fanmingming demo.m3u（实测为无流地址的空壳列表）和 hujingguang/ChinaIPTV（实测流地址全部失效）；v2.5.0 起移除被墙的 jsDelivr 镜像，GitHub 源自动经国内镜像加速；v2.6.0 起移除失效的 YueChan APTV（404）与被区域封锁的 wcb1969（GitHub 451），替换为 best-fan 省份/央视专项源；v2.7.0 起新增咪咕/百视通专项源（解析时自动过滤咪咕 VOD 回放与历年春晚/更新时间等垃圾分组）；v2.8.0 起新增 ChinaIPTV cnTV1_ALL、vbskycn iptv4、vicjl TV-IPV4 三个源，并过滤影视资源站/斗鱼虎牙循环/央视点播片段/快手视频文件等点播噪音；v2.9.0 起修复频道漂移/跳台/广告源混入，过滤广告网络域/广播电台/咪咕虚拟直播，未分类从 115 降至 3。
